...
/**
 * Clase AutorController: Gestiona las peticiones HTTP relacionadas con las Categorías.
 */
class AutorController
{
    ...
    {
        $data = json_decode(file_get_contents("php://input"));
        if (!empty($data->id) && !empty($data->nombre) && !empty($data->nacionalidad)) {
    ...
    public function destroy()
    {
        ...
        if (!empty($data->id)) {
            if ($this->autorService->delete($data->id)) {
                ...
                http_response_code(503);
                echo json_encode(['message' => 'No se pudo eliminar el autor.']);
            }
        } else {
            http_response_code(400);
